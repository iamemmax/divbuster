...
const LogoutModal = ({
  ...
  return (
    <Dialog open={showSuccessModal}>
      <DialogContent className="!overflow-hidden w-full md:w-[26rem] h-[300px] border-[0.01px] border-opacity-90 border-[#407BFF] ">
        <DialogBody className="bg-[#141B3f] w-full  ">
          <div className="py-1 pb-4">
            ...
            <div className="w-full px-6 flex justify-center py-3 items-center gap-[1rem] mt-[1rem] text-base">
                <Button variant={"outlined"} className="rounded-3xl w-full border-white  py-[0.9rem] text-white  shadow-lg transition-colors delay-150 ease-in-out focus:outline-none" onClick={()=>setShowSuccessModal(false)}>
                Cancel
                </Button>
                <Button className="rounded-3xl w-full bg-white  border-[0.3px]   py-[0.9rem]  text-primary shadow-lg transition-colors delay-150 ease-in-out focus:outline-none" onClick={handleLogoutClick}>
                Log out
                </Button>
            </div>
          </div>
