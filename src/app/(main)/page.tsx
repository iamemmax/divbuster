...
import GenerateReferralModal, {
  ReferralsuccessProp,
} from "./misc/components/insurance/modals/referral/GenerateReferralModal";
...
export default function Home() {
  const [showGenerateReferralModal, setShowGenerateReferralModal] =
    useState(false);
  const [
    showGenerateReferralSuccessModal,
    setShowGenerateReferralSuccessModal,
  ] = useState(false);
  const [referralResponse, setReferralResponse] =
    useState<ReferralsuccessProp>();
          ...
          <div className="flex flex-1 flex-col  gap-4  md:gap-6  max-md:py-10">
            ...
            <div className="flex items-center   gap-2 md:gap-4 ">
              ...
              >
                ...
                <span className="  flex items-center bg-white justify-center p-1 sm:p-2 rounded-full ml-4 md:ml-7">
                  <RightUpArrow
                    className=""
                    height={12}
                    width={12}
            ...
            <div className="bg-[#1E2954] relative p-6 py-[2rem]  rounded-[20px]">
              <div className=" grid grid-cols-1 md:grid-cols-2 items-center gap-4">
                {cardArray1?.map((card, id: number) => (
                  <div
                    key={id}
                    className="bg-[#161d42] py-11 px-[15px] rounded-lg flex justify-center items-center"
                  >
            ...
            <div className="bg-[#1E2954] p-6 py-[2rem]  rounded-[20px]">
              <div className=" grid grid-cols-1 md:grid-cols-2 items-center gap-4">
                {cardArray2?.map((card, id: number) => (
                  <div
                    key={id}
                    className="bg-[#161d42] py-4 px-6 xl:px-[1.6rem] rounded-lg flex flex-col justify-center items-center"
                  >
                    <h3 className="text-lg font-medium text-white">
                      {card?.title}
                    </h3>
                    <p className="text-sm text-[#CAC9D4] text-center">
                      {card.desc}
                    </p>
                    <LinkButton
                      href={"/about-us"}
