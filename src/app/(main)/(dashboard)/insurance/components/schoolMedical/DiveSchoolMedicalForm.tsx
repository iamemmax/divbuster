...
import { useSubmitMedalSchoolReport } from '../../../api/insurance/school-medical/submitSchoolMedicalForm'
...
const DiveSchoolMedicalForm = ({ onSuccess }: DiveSchoolLiabilityFormProps) => {
  const {
  ...
  const { mutate: handleSubmitDiveSchoolLiability, isLoading: isSubmitting } = useSubmitMedalSchoolReport()
  ...
  const onSubmit = (data: DiveSchoolLiabilityFormData) => {
    if (sigRef.current && !sigRef.current.isEmpty() ) {
      ...
      handleSubmitDiveSchoolLiability({ payload }, {
        onSuccess: () => {
          toast.success('Dive School Medical form submitted successfully')
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <Button
        type="submit"
        disabled={!isSigned  || !acceptTerms || !diveSchoolId}
        className={`w-full flex items-center justify-center gap-x-3 py-3 rounded-md ${
          isSigned  && acceptTerms && diveSchoolId
...
export default DiveSchoolMedicalForm
