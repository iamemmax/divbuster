...
const CreateDivePlanBuddyBooking = ({setStep,buddyMembers,setBuddyMembers,onClose,stepOneLogDetails}:prop) => {
      const {
          isErrorModalOpen,
          setErrorModalState,
  ...
  // Submit email and create booking
  const onSubmit = (data: AdvancedDetails) => {
    ...
    if (!emailList.includes(data.email)) {
      finalEmailList = [...emailList, data.email];
      setEmailList(finalEmailList);
    }

    // Validate that we have at least one email or buddy selected
    if (finalEmailList.length === 0 && selectedBuddies.size === 0) {
      toast.error("Please add at least one email or select a buddy");
      return;
    }

    // Create the booking with the final email list
    createBooking(finalEmailList);
  };
