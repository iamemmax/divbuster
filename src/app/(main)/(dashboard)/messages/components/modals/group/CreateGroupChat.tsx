    ...
    <div className="max-w-4xl mx-auto bg-[#f9fafb] dark:bg-[#1A1D21] rounded-2xl shadow p-8 space-y-8">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_2fr] border-b border-[#EAECF0] dark:border-gray-600 border-opacity-50 py-4 md:py-2 items-start gap-6 md:gap-5 mt-4">
          ...
          <input
            className="border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 outline-none h-11 rounded-10 px-3 py-2 w-full text-sm text-gray-900 dark:text-gray-100"
          ...
          {suggestedMembers?.map((buddy) => (
            <div key={buddy.id} className="text-center shrink-0">
              {buddy.avatar ? (
                ...
                <div className="md:w-16 md:h-16 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 font-medium flex items-center justify-center mx-auto">
                  {buddy?.name
                    .split(" ")
                    .map((n) => n[0])
                    .join("")
              ...
              <p className="text-xs font-archivo text-[#98A2B3] dark:text-gray-400 mt-2">
                {buddy?.name}
              </p>
            </div>
          ))}
