...
const GroupMessageBox = ({ onSendMessage, groupChats,onBackToRecent, selectedGroup }: MessageBoxProps) => {
  const me: User = {
    id: 99,
    name: "You",
    username: "@you",
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h2 className="font-semibold text-gray-900">{selectedGroup?.groupName}</h2>
            </div>
          ...
          <div className='max-h-[52vh] md:max-h-[58vh] overflow-y-auto'>
            {selectedGroup?.messages?.map((msg:Message, index:number) => {
              ...
              const prevMessageDate =
                index > 0 ? new Date(selectedGroup.messages[index - 1].timestamp).toDateString() : null
                ...
                <div key={index} className='py-6'>
                  ...
                  <div className={`flex ${isCurrentUser ? 'justify-end' : 'items-start space-x-3'}`}>
                    ...
                    <div className={`flex-1 ${isCurrentUser ? 'flex flex-col items-end' : ''}`}>
                      <div className={`flex items-center gap-x-6 mb-1 ${isCurrentUser ? 'flex-row-reverse' : ''}`}>
