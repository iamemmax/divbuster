...
export function AuthenticationComponent({language}:prop) {
    const t = authenticationTranslations[language] || authenticationTranslations?.en;
    const passwordChangeSchema = z
      .object({
        currentPassword: z.string().min(1, t.validation.currentRequired),
        newPassword: z
          .string()
          .min(8,t.validation.newRequired)
          .regex(
            /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)/,
            t.validation.newStrength
          ),
  ...
  const [showNewPassword, setShowNewPassword] = React.useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = React.useState(false);
  const { mutate: handleUpdatePassword, isLoading: isSubmitting } =
  ...
  return (
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        {/* Current Password */}
        <div>
          <label className="block text-xs font-archivo font-medium mb-2">
            {t.currentPassword}
          </label>
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-3 border rounded-lg pr-12 dark:bg-gray-800 dark:text-white"
            />
            <button
              type="button"
              onClick={() => setShowCurrentPassword(!showCurrentPassword)}
