...
const DiverBuddies = () => {
  ...
  return (
    <div>
      ...
      <div className="w-full bg-white min-h-screen px-4 sm:px-6 lg:px-[1.875rem]">
        {/* Header */}
        <div className="bg-white py-4 border-gray-200">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
              <button className="flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition-colors font-medium text-sm">
                Favorite Buddy
              </button>
              <button className="flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition-colors font-medium text-sm">
                Buddy Requests
              </button>
            </div>
          </div>
        ...
        <div className="flex flex-col xl:flex-row gap-6 xl:gap-10">
          {/* Main Content */}
          <div className="flex-1 p-4 sm:p-6 border border-[#EAECF0] rounded-lg">
            {/* Section Header */}
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
              <div className="flex items-center gap-3">
                <h1 className="text-xl sm:text-2xl font-semibold text-gray-900">Dive Buddies</h1>
                <div className="bg-[#FEF6F4] rounded-[1rem] flex justify-center items-center px-2 py-[.1875rem]">
                  <span className="text-[#F7931D] text-xs font-archivo font-medium">48 users</span>
                </div>
              </div>
              
              <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                <button className="flex items-center gap-2 px-3 sm:px-4 py-2 border text-[#344054] font-archivo font-medium text-xs sm:text-sm border-gray-300 rounded-lg hover:bg-gray-50 transition-colors flex-1 sm:flex-none justify-center">
                  <UserGroupIcon />
                  <span className="hidden sm:inline">Create Group</span>
                  <span className="sm:hidden">Group</span>
                </button>
                <button className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-[#F7931D] text-white font-archivo font-medium text-xs sm:text-sm rounded-lg hover:bg-orange-600 transition-colors flex-1 sm:flex-none justify-center">
                  <PlusIcon />
            ...
            <div className="hidden xl:block bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
              <div className="grid grid-cols-4 gap-4 px-6 py-4 border-b border-gray-200 bg-gray-50 text-sm font-medium text-gray-700">
                <div>Name</div>
              ...
              <div className="divide-y divide-gray-200">
                {diveBuddies.map((buddy) => (
                  <div key={buddy.id} className="grid grid-cols-4 gap-4 px-6 py-4 items-center hover:bg-gray-50 transition-colors">
                    {/* Name Column */}
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="font-medium text-[#101828] text-sm font-archivo truncate">{buddy.name}</div>
                        <div className="text-sm text-[#667085] font-archivo truncate">{buddy.email}</div>
                      </div>
                    </div>
                    ...
                    <div className="text-right">
                      <Link href={`/div-buddies/profile/${buddy.id}`} className="text-orange-500 font-archivo text-sm hover:text-orange-600 font-medium whitespace-nowrap">
                        View Profile
                      </Link>
                    </div>
                  </div>
            ...
            <div className="xl:hidden space-y-4">
              {diveBuddies.map((buddy) => (
                <div key={buddy.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow">
                  {/* Header */}
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="font-medium text-[#101828] text-base font-archivo">{buddy.name}</div>
                        <div className="text-sm text-[#667085] font-archivo truncate">{buddy.email}</div>
                      </div>
                    </div>
                    <button className="text-orange-500 font-archivo text-sm hover:text-orange-600 font-medium flex-shrink-0 ml-2">
                      View Profile
                    </button>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {/* Dive Buddies */}
                    <div>
                      <div className="text-xs text-gray-500 font-medium mb-2 uppercase tracking-wide">Dive Buddies</div>
                      <div className="flex items-center gap-2">
                        {buddy.buddyCount > 0 ? (
                          <div className="flex -space-x-1">
                          ...
                          <span className="text-sm text-gray-400">No buddies yet</span>
                        )}
                      </div>
                    </div>
                    ...
                    {/* Certificates */}
                    <div>
                      <div className="text-xs text-gray-500 font-medium mb-2 uppercase tracking-wide">Certificates</div>
           ...
           {/* Pagination */}
            <div className="flex items-center justify-between mt-6 gap-4">               
              <button                  
                className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50 justify-center"                 
                disabled={currentPage === 1}
                onClick={() => setCurrentPage(currentPage - 1)}               
              >                 
               <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M12.8327 6.99996H1.16602M1.16602 6.99996L6.99935 12.8333M1.16602 6.99996L6.99935 1.16663" stroke="#344054"  stroke-width="1.67" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
              ...
              >                 
                Next                 
                <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M1.16602 6.99996H12.8327M12.8327 6.99996L6.99935 1.16663M12.8327 6.99996L6.99935 12.8333" stroke="#344054" stroke-width="1.67" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
          ...
          <div className="w-full xl:w-80 bg-white border border-gray-200 rounded-lg p-4 sm:p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-6">Suggested Buddies</h2>
